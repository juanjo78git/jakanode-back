...
def format_user_with_auths(
    user_data, telegram_data=None, google_data=None, auth_providers=None
):
    """
    ...
    if user_data:
        return {
            ...
            "auth_methods": {
                "telegram": (
                    format_telegram_data(telegram_data) if telegram_data else None
                ),
                "google": format_google_data(google_data) if google_data else None,
                "providers": (
                    [format_auth_provider_data(p) for p in auth_providers]
                    if auth_providers
                    else []
                ),
            },
